...
const EngineUI = (() => {
  ...
  async function boot() {
    ...
    // Pull this account's learned weights + feedback count from the backend
    try {
      const { user } = await API.auth.getMe();
      if (user.signalWeights) LearnSystem.saveWeights(user.signalWeights);
      API.auth.setUser(user);
      log(`ACCOUNT SYNCED — ${user.feedbackCount || 0} PRIOR FEEDBACK ENTRIES`, 'ok');
    } catch (e) {
      log('⚠ COULD NOT SYNC ACCOUNT — USING LOCAL WEIGHTS', 'w');
    }
  ...
  async function runAnalysis() {
    ...
    lastScanData = { isFake, fake, S: { ...S }, scanId: null };
    saveScan(isFake, fake, S, analysisTimeMs);
  }
  ...
  // ── SAVE SCAN (persisted to backend — tied to the logged-in account) ──
  async function saveScan(isFake, fake, S, analysisTimeMs) {
    ...
    const payload = {
      ...
      signals: {
        pixel: S.pixel, fft: S.fft, ela: S.ela, face: S.face, color: S.color,
        hand: S.hand, lighting: S.lighting, background: S.background, textgeo: S.textgeo,
      },
      weightsUsed:   LearnSystem.loadWeights(),
      analysisTimeMs,
    };
    try {
      const { scan } = await API.scans.save(payload);
      lastScanData.scanId = scan._id;
      log(`SCAN SAVED TO ACCOUNT — #${scan._id.slice(-6).toUpperCase()}`, 'ok');
    } catch (e) {
      log('⚠ COULD NOT SAVE SCAN TO ACCOUNT — CHECK CONNECTION', 'e');

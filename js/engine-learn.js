...
const LearnSystem = (() => {
  ...
  async function submitFeedback(wasCorrect) {
    ...
    if (!scan) return;
    const { isFake, S, scanId } = scan;

    document.getElementById('fbCorrect').disabled = true;
    document.getElementById('fbWrong').disabled   = true;

    // The backend runs the identical weight-adjustment algorithm and is the
    // source of truth for the account's learned weights. Fall back to a
    // local-only computation if the scan couldn't be saved (e.g. offline).
    let newW = null;
    if (scanId) {
      try {
        const res = await API.scans.feedback(scanId, wasCorrect);
        newW = saveWeights(res.updatedWeights);
        const user = API.auth.getUser();
        if (user) { user.feedbackCount = (user.feedbackCount || 0) + 1; API.auth.setUser(user); }
      } catch (e) {
        console.error('Backend feedback sync failed, using local fallback:', e);
      }
    }
    if (!newW) {
